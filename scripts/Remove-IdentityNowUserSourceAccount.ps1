...
#>
    ...
    if ($v3Token.access_token) {
        try {                         
            $deleteAccount = Invoke-RestMethod -Method Delete -Uri (Get-IdentityNowOrgUrl v2 "/accounts/$($account)?org=$($IdentityNowConfiguration.orgName)") -Headers @{Authorization = "$($v3Token.token_type) $($v3Token.access_token)"; "Content-Type" = "application/json"} 
