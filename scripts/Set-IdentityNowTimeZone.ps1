...
#>
    ...
    if ($v3Token.access_token) {
        try {    
            ...
            $updateTZ = Invoke-RestMethod -Method Patch -Uri (Get-IdentityNowOrgUrl beta "/org-config") -Headers @{Authorization = "$($v3Token.token_type) $($v3Token.access_token)"; "Content-Type" = 'application/json-patch+json' } -Body $tzBody 
            return $updateTZ            
        }
        catch {
