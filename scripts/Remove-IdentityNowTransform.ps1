...
#>
    ...
    if ($v3Token.access_token) {
        try {
            $IDNDeleteTransform = Invoke-RestMethod -Method Post -Uri (Get-IdentityNowOrgUrl v1 "/transform/delete/$($ID)") -Headers @{Authorization = "$($v3Token.token_type) $($v3Token.access_token)" } 
