...
#>
    ...
    if ($v3Token.access_token) {
        try {
            $IDNNewTransform = Invoke-RestMethod -Method Post -Uri (Get-IdentityNowOrgUrl v1 "/transform/create") -Headers @{Authorization = "$($v3Token.token_type) $($v3Token.access_token)"; "content-type" = "application/json" } -Body $transform
