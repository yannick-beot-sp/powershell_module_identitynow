...
#>
    ...
    if ($v3Token.access_token) {
        try {                         
            $updateAccount = Invoke-RestMethod -Method Patch -Uri (Get-IdentityNowOrgUrl v2 "/accounts/$($account)?org=$($IdentityNowConfiguration.orgName)") -Headers @{Authorization = "$($v3Token.token_type) $($v3Token.access_token)"; "Content-Type" = "application/json"} -Body $update
