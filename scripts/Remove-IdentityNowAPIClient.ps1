...
#>
    ...
    if ($v3Token.access_token) {
        try {
            $IDNDeleteAPIClient = Invoke-RestMethod -Method Post -Uri (Get-IdentityNowOrgUrl cc "/client/remove/$($ID)") -Headers @{Authorization = "$($v3Token.token_type) $($v3Token.access_token)"; "Content-Type" = "application/json"} 
