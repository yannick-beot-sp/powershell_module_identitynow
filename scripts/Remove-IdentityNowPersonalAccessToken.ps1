...
#>
    ...
    if ($v3Token.access_token) {
        try {    
            $IDNDeletePAT = Invoke-RestMethod -Method Delete -Uri (Get-IdentityNowOrgUrl beta "/personal-access-tokens/$($id)") -Headers @{Authorization = "$($v3Token.token_type) $($v3Token.access_token)" }
            return $IDNDeletePAT
        }
        catch {
