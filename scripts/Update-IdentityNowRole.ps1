...
#>
    ...
    if ($v3Token.access_token) {
        try {
            $IDNUpdateRole = Invoke-RestMethod -Method Post -Uri (Get-IdentityNowOrgUrl cc "/role/update") -Headers @{Authorization = "$($v3Token.token_type) $($v3Token.access_token)"; "content-type" = "application/json" } -Body $update
