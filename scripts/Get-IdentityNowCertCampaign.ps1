...
#>
    ...
    if ($v3Token.access_token) {
        try {
            if ($campaignID) {           
                $IDNCertCampaigns = Invoke-RestMethod -Method Get -Uri (Get-IdentityNowOrgUrl cc "/campaign/getCertifications?_dc=$($utime)&campaignId=$campaignID") -Headers @{Authorization = "$($v3Token.token_type) $($v3Token.access_token)" }            
                return $IDNCertCampaigns.items
            } else {
                $IDNCertCampaigns = Invoke-RestMethod -Method Get -Uri (Get-IdentityNowOrgUrl cc "/campaign/list?_dc=$($utime)&completedOnly=$($completed)&start=0&limit=$($limit)") -Headers @{Authorization = "$($v3Token.token_type) $($v3Token.access_token)" }            
                return $IDNCertCampaigns.items
            }               
        }
        catch {
