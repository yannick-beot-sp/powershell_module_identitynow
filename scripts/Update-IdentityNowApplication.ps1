...
#>
    ...
    if ($v3Token.access_token) {
        try {         
            ...
            $IDNApps = Invoke-RestMethod -Method Post -Uri (Get-IdentityNowOrgUrl cc "/app/update/$($appID)") -Headers @{Authorization = "$($v3Token.token_type) $($v3Token.access_token)"; "Content-Type" = "application/json" } -Body $update 
            return $IDNApps
        }
        catch {
