    ...
    if ($v3Token.access_token) {
        try {
            if ($ID) {
                ...
                $IDNAPIClient = Invoke-RestMethod -Method Get -Uri (Get-IdentityNowOrgUrl beta "/oauth-clients/$($ID)?_dc=$($utime)") -Headers @{Authorization = "$($v3Token.token_type) $($v3Token.access_token)"; "Content-Type" = "application/json" }
                return $IDNAPIClient
            }
            else {
                $utime = [int][double]::Parse((Get-Date -UFormat %s))
                $IDNAPIClient = Invoke-RestMethod -Method Get -Uri (Get-IdentityNowOrgUrl beta "/oauth-clients?_dc=$($utime)") -Headers @{Authorization = "$($v3Token.token_type) $($v3Token.access_token)"; "Content-Type" = "application/json" }
                return $IDNAPIClient
            }
        }
        catch {
