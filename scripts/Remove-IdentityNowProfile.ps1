...
#>
    ...
    if ($v3Token.access_token) {
        try {   
            ...
            $IDNDeleteIDP = Invoke-RestMethod -Method Post -Uri (Get-IdentityNowOrgUrl cc "/profile/bulkDelete") -Headers @{Authorization = "$($v3Token.token_type) $($v3Token.access_token)" ; "Content-Type" = "application/x-www-form-urlencoded; charset=UTF-8"} -Body $profID
            return $IDNDeleteIDP
        }
        catch {
