...
#>
    ...
    if ($v3Token.access_token) {        
        try {   
            $accountActivites = Invoke-RestMethod -Method Get -URI (Get-IdentityNowOrgUrl v3 "/account-activities/$($id)") -Headers @{Authorization = "Bearer $($v3Token.access_token)"; "Content-Type" = "application/json" }
