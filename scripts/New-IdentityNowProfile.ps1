...
#>
    ...
    if ($v3Token.access_token) {
        try {
            ...
            $IDNProfile = Invoke-RestMethod -Method Post -Uri (Get-IdentityNowOrgUrl cc "/profile/create") -Headers @{Authorization = "$($v3Token.token_type) $($v3Token.access_token)" } -Body $body
            return $IDNProfile
        }
        catch {
