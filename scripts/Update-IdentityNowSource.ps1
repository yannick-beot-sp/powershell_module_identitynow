...
#>
  ...
  if ($v3Token.access_token) {
    try {                         
      if ($accountProfile) {
        $body = $update
        $updateSource = Invoke-RestMethod -Method Post -Uri (Get-IdentityNowOrgUrl v1 "/accountProfile/bulkUpdate/$($sourceID)") -Headers @{Authorization = "$($v3Token.token_type) $($v3Token.access_token)"; "Content-Type" = "application/json" } -Body $body
        return $updateSource
      }
      else {
        Write-Verbose "update ===> $($update)"
        $updateSource = Invoke-RestMethod -Method Post -Uri (Get-IdentityNowOrgUrl cc "/source/update/$($sourceID)?$($update)") -Headers @{Authorization = "$($v3Token.token_type) $($v3Token.access_token)"; "Content-Type" = "application/json" }            
        return $updateSource
      }          
    }
    catch {
