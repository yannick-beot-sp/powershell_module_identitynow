...
#>
    ...
    if ($v3Token.access_token) {
        try {
            $IDNProfile = Invoke-RestMethod -Method Post -Uri (Get-IdentityNowOrgUrl cc "/profile/refresh/$($ID)" )-Headers @{Authorization = "$($v3Token.token_type) $($v3Token.access_token)" }
            return $IDNProfile
        }
        catch {
