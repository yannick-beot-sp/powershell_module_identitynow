...
#>
    ...
    if ($v3Token.access_token) {
        try {
            $updateProfile = Invoke-RestMethod -Method Post -uri (Get-IdentityNowOrgUrl v1 "/profile/update/$($ID)") -Headers @{Authorization = "$($v3Token.token_type) $($v3Token.access_token)"; "Content-Type" = "application/json" } -Body (@{"priority" = $priority } | convertto-json)            
